...
Inside `${HOME}/bioit/apps/abyss/SPEC` there is a script called `build`. This just requires the version number and will download, compile, install and create the modulefile for you. Execute it as follows:
    ...
    ${HOME}/bioit/apps/abyss/SPEC/build 2.2.5
...
Download the source tarball from [here](https://github.com/bcgsc/abyss/releases/download/2.2.5/abyss-2.2.5.tar.gz) into `/opt/bioit/abyss/src`, untar it and cd into the resulting directory then run the following:
...
Now configure and build:
    ...
    ./configure --prefix=/opt/bioit/abyss/2.2.5 --disable-werror
    make clean
    make
    make install
...
Add a module file in `/opt/bioit/modulefiles/abyss/` for this version by copying previous ones and modifying the paths.
    ...
    #
    prepend-path  PATH         /opt/bioit/abyss/2.2.5/bin
    prepend-path  MANPATH      /opt/bioit/abyss/2.2.5/share/man/man1
