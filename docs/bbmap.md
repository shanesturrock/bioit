...
Inside `${HOME}/bioit/apps/bbmap/SPEC` there is a script called `build`. This just requires the version number and will download, compile, install and create the module file for you. Execute it as follows:
    ...
    ${HOME}/bioit/apps/bbmap/SPEC/build 39.91
...
Add a module file in `/opt/bioit/modulefiles/bbmap/` for this version by copying previous ones and modifying the paths.
    ...
    #
    prepend-path  PATH         /opt/bioit/bbmap/39.91

## RPM

There's a SPEC file for this package in `${HOME}/bioit/apps/bbmap/SPEC` so modify that with the new version details. The `/opt/bioit/bbmap/39.91/doc/changelog.txt` will give you the details for the `%changelog` section of the SPEC file.
...
Before you install this, you need to sign it using:
    ...
    rpm --addsign \
    ${HOME}/rpmbuild/RPMS/x86_64/bbmap-39.91-1.el7.bioit.x86_64.rpm

Now you can move it to `/opt/bioit/repo/RPMS`

    mv ${HOME}/rpmbuild/RPMS/x86_64/bbmap-39.91-1.el7.bioit.x86_64.rpm \
    /opt/bioit/repo/RPMS
