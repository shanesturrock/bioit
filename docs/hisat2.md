...
Inside `${HOME}/bioit/apps/hisat2/SPEC` there is a script called `build`. This just requires the version number and will download, compile, install and create the modulefile for you. Execute it as follows:
    ...
    ${HOME}/bioit/apps/hisat2/SPEC/build 2.2.3
...
If that shows as being there you can test it works with:
    ...
    module load hisat2/2.2.3
    which hisat2
    hisat2 --version

## Manual Build

Download the version to be built into `/opt/bioit/hisat2/src` and unzip. Here we'll use version 2.2.3.
...
This just builds in the current location. Since it isn't good to have the binaries in the same location as the source, you first need to move docs, examples and scripts:
    ...
    mv scripts /opt/bioit/hisat2/2.2.3/
    mv docs /opt/bioit/hisat2/2.2.3/
    mv example /opt/bioit/hisat2/2.2.3/

Now you need to move all the binaries and python packages into `/opt/bioit/hisat2/2.2.3/bin` as follows:

    mkdir /opt/bioit/hisat2/2.2.3/bin
    mv hisat2 /opt/bioit/hisat2/2.2.3/bin/
    mv hisat2-* /opt/bioit/hisat2/2.2.3/bin/
    mv *.py /opt/bioit/hisat2/2.2.3/bin/
...
Add a module file in `/opt/bioit/modulefiles/hisat2/` for this version by copying previous ones and modifying the path.
    ...
    #
    prepend-path  PATH         /opt/bioit/hisat2/2.2.3/bin
