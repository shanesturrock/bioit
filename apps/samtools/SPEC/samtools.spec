...
alternatives \
   ...
   --slave %{_bindir}/plot-ampliconstats plot-ampliconstats /opt/bioit/%{name}/%{version}/bin/plot-ampliconstats \
...
%changelog
* Wed Feb 22 2023 Shane Sturrock <shane.sturrock@gmail.com> - 1.18-1
...
* Tue Sep 06 2022 Shane Sturrock <shane.sturrock@gmail.com> - 1.16.1-1
...
- New samtools ampliconclip sub-command for removing primers from
  ...
  projects. The primers are listed in a BED file and can be either soft-clipped
  or hard-clipped. (#1219)
- New samtools ampliconstats sub-command to produce a textual summary of primer
  and amplicon usage, in a similar style to samtools stats. The
  misc/plot-ampliconstats script can generate PNG images based on this text
  report. (#1227)
- Samtools fixmate, addreplacerg, markdup, ampliconclip and sort now accept a
  -u option to enable uncompressed output, which is useful when sending data
  over a pipe to another process. Other subcommands which already support this
  option for the same purpose are calmd, collate, merge, view and depad. (#1265)
...
- samtools sort now has a minhash collation (-M) to group unmapped reads with
  similar sequence together. This can sometimes significantly reduce the file
  size. (#1093)
- samtools bedcov now has -g and -G options to filter-in and filter-out based
  on the FLAG field. Also the new -d option adds an extra column per file
  counting the number of bases with a depth greater than or equal to a given
  threshold. (#1214)
- Fixed samtools bedcov -j option (discard deletions and ref-skips) with
  multiple input files (#1212)
- samtools bedcov will now accept BED files with columns separated by spaces as
  well as tabs (#1246; #1188 reported by Mary Carmack)
- samtools depth can now include deletions (D) when computing the base coverage
  depth, if the user adds the -J option to the command line (#1163).
- samtools depth will count only the bases of one read, for the overlapping
  section of a read pair, if the -s option is used in the command line (#1241,
  thanks to Teng Li).
...
- samtools view now accepts remote URIs for FASTA and FAI files. Furthermore,
  the reference and index file can be provided in a single argument, such as
...
- samtools faidx gets new options --fai-idx and --gzi-idx to allow
  specification of the locations of the .fai and (if needed) .gzi index files.
  (#1283)
- The samtools fasta/fastq -T option can now add SAM array (type B) tags to the
  output header lines. (#1301)
- samtools mpileup can now display MAPQ either as ASCII characters (with
  ...
  previous versions) or comma-separated numbers (with --output-extra MAPQ; in
  SAM column order alongside other selected --output-extra columns).
- When both -s/--output-MQ and -O/--output-BP are used, samtools 1.10 printed
  the extra columns in the opposite order. This changes the format produced by
  1.10's --output-extra MAPQ. (#1281, thanks to John Marshall; reported by
  Christoffer Flensburg)
- samtools tview now accepts a -w option to set the output width in text mode
  (-d T). (#1280)
- The dict command can now add AN tags containing alternative names with "chr"
  prefixes added to or removed from each sequence name as appropriate and
  listing both "M" and "MT" alternatives for mitochondria. (#1164, thanks to
  John Marshall)
...
- Additional documentation improvements, thanks to John Marshall (#1181; #1224;
  #1248; #1262; #1300)
...
- The samtools sort tests now use less memory so the test suite works better on
  small virtual machines. (#1159)
...
- Fixed samtools coverage minor inconsistency vs idxstats (#1205; #1203
  reported by @calliza)
- Fixed samtools coverage quality thresholding options which were the wrong way
  round compared to mpileup (-q is the mapping quality threshold and -Q is base
  quality). (#1279; #1278 reported by @kaspernie)
- Fixed bug where samtools fastq -i would add two copies of the barcode in the
  fastq header if both reads in a pair had a BC:Z tag (#1309; #1307 reported by
  @mattsoup)
- samtools calmd no longer errors with a SEQ of * (#1230; #1229 reported by Bob
  Harris)
...
- Fixed samtools stats integer overflows and added support for long references
  (#1174; #1173)
